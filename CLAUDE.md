...
**モックは見本として凍結・触らない**：`稼働管理\ECS\ECS_モック\*.html`、および本フォルダ内の `public/ecs/*.html`・`public/ecs/data/*.js`（実体はBladeが動く＝本実装の参照元ではない）。

## 作業を始める前に、必ず下記を読むこと（設計の「正」はすべてOneDriveにある。コピーは作らない＝唯一の正）
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\エンジニア引き継ぎ資料.md`   ← 全体の入口
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\作業リスト_共有.md`          ← 現在地・次にやること・設計の宿題
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\ECS_設計書_v1.1.md`        ← 要件・データ設計の本体（正）
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\ECS_図解_v1.1.md`          ← 図解（Mermaid）
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\エンジニア確認事項_Laravel.md`  ← 技術の確定事項＋MTGで決めたいこと
- `C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\要検討事項まとめ.md`           ← 決定事項の一覧
