...
## チャットワークに送る文章・指示書を用意するとき
- 置き場所は **`C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\`** の1か所だけ。**新しい場所を作らない。**
  - チャットワークに送る文章＝`C:\Users\onuma\OneDrive\Desktop\稼働管理\ECS\チャットワーク送信用.txt`（毎回上書きでOK。上書きしたら前に何が入っていたかを一言報告する）
  - 別ターミナル用・上長用の指示書、スプシへの貼り付け用テキストも**同じフォルダ**に置く。
  - ⚠ **`C:\Users\onuma\稼働管理\` という同名の別フォルダがあるが、そちらは偽物**（2026-08-28にできたもの）。
    そこに書くと小沼さんが見つけられない。2026-09-07に実際に取り違えた。**書く前に `ls` で上のパスを確かめる。**
