...
final class RecruitStatus
{
    ...
    {
        if (! $project->staff_published || ! $project->is_recruiting) {
            return '';
        }
    ...
    /**
     * 【この人数で足りている（募集を締める）】について（2026-09-01 baba決定）
     *
     * 運営人数はセールスが書いた数字で、**そのまま残す**（8名と書いてあれば8名のまま）。
     * 実際には「6名で回せそう」とアサイン担当が判断して、**人数が埋まっていなくても
     * これで決まりにする**運用がある。そのとき「募集中 あと2名」が残っていると、
     * **まだ人を足すのか／もう要らないのかが分からない**（スタッフからのご意見）。
     *
     * ⚠ **「確定にする」では締めない。** 確定にしても
     *   **本当に人が足りていなくて募集を続けたい案件がある**（baba）。
     *   締めるかどうかは別の意思表示なので、日別ボードの
     *   **「🔒 この人数で足りている」ボタン**（`is_recruiting` を false にする）で人が決める。
     * ⚠ バッジを消すだけにしてはいけない。社員には見えないのに**スタッフ画面には募集が出続けて
     *   応募できる**状態になる（2026-08-28 に同じズレで不具合になっている）。必ず実体を変える。
     * ⚠ 締めたあとに足したくなったら「＋ 追加募集する」で戻せる（確定後の追加募集はよくある・baba）。
     */
}
