...
class OfficeScope
{
    ...
    /**
     * 「ほかの拠点に巻き取られた案件」を、登録拠点の画面から外す（2026-09-16 baba要望）。
     *
     * 巻き取り＝その拠点が運営を引き取る、という意味（ヘルプ＝人だけ出す、とは違う）。
     * 引き取られた側は人を入れないので、**日別ボードに並んでいると邪魔になる**。
     *
     * ⚠ 使うのは**日別ボードとスタッフ公開ボードだけ**（2026-09-16 baba決定）。
     *   公開ボードも外す理由＝引き取られた案件を、登録拠点が自分のスタッフに
     *   募集として出してしまうのを防ぐため。
     *   案件一覧・集計・収支からは消さない。消すと
     *   ・巻き取りを「解除」する入口（案件一覧）にたどり着けなくなる
     *   ・登録拠点の売上・実績が画面から消える
     *   ⚠ ほかの画面にも広げたくなったら、必ず baba に確かめてから。
     *
     * ⚠ $office が null（全拠点）のときは何もしない＝全部見せる。
     *
     * しくみ：その案件に「自分の拠点**以外**への巻き取り」があるなら外す。
