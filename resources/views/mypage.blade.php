...
@endverbatim
      ...
      @if ($errors->any())
        <div class="panel mp-wrap" style="margin-top:12px; background:#fdecec; color:#b91c1c; border-color:#f3c0c0; font-size:13px; padding:12px 14px;">
          {{ $errors->first() }}
        </div>
      @endif
        ...
        <form method="POST" action="/profile/extras" style="padding:4px 2px 2px;">
          ...
          <div class="mp-prof-grid">
            ...
            <div class="mp-prof-val">
              <textarea id="pxNote" name="profile_note" rows="2"
                        placeholder="例）運転練習中です。簡単な動画編集ができます。">{{ $extras['profile_note'] }}</textarea>
            </div>
          </div>
          ...
          <div style="margin-top:12px;">
            ...
            <span class="mp-prof-hint" style="margin-left:10px;">
              氏名・所属・身長などは、上のカードの「氏名・所属などを編集」から直せます。
            </span>
          </div>
        </form>
      </div>
