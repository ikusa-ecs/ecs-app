...
@verbatim
      ...
      <div class="panel">
        ...
        <div class="cols-help" style="margin-top:16px;">
          ...
          <u>当日・手配</u>　<code>担当体制</code>　<code>集合時間</code>（08:00）　<code>解散時間</code>　<code>イベント入場</code>　<code>イベント開始</code>　<code>イベント終了</code>　<code>運営人数</code>（数字／<code>6〜8</code>のような幅もOK。<b>空でもOK＝参加人数とコンテンツから「仮」で入ります・最少5名</b>）　<code>お客様人数</code>　<code>チーム数</code>　<code>リピート</code>（あり/なし）　<code>音響機材</code><br>
...
<script>
  ...
  function validate(r) {
    ...
    // ⚠ 2026-09-08 baba要望：**空欄はエラーにしない**＝参加人数とコンテンツから「仮」で入る。
    //   書いてあるのに読めないときだけエラー（勘で埋めないため）。
    if (String(r.count == null ? '' : r.count).trim() !== '' && impMaxCount(r.count) < 1) {
      errs.push('運営人数が読めません（例 16 ／ 6〜8）');
    }
    return errs;
  }

  // 運営人数が空の行か（＝取り込むと「仮」の人数が入る）。
  // ⚠ 何名になるかはサーバーが決める（コンテンツの必要人数を見るため、画面では分からない）。
  //   正本＝App\Support\RequiredCountEstimate。ここに人数の計算を書かない。
  function isCountBlank(r) {
    return String(r.count == null ? '' : r.count).trim() === '';
  }
      ...
      tr.innerHTML = `
        ...
        <td>${isOk
          ? '<span class="res-ok">✓ OK</span>'
            + (isCountBlank(r)
              ? '<br><span class="err-detail">運営人数は「仮」で入ります（参加人数とコンテンツから算出・最少5名）。あとで直してください</span>'
              : '')
