...
<style>
  ...
  html[data-theme="dark"] .pj-flash.ok { color: var(--ok-ink); border-color: var(--ok-line); }
  html[data-theme="dark"] .pj-flash.err { background: var(--danger-soft); color: var(--danger-ink); border-color: var(--danger-line); }
  html[data-theme="dark"] .pj-flash.warn { background: var(--warn-soft); color: var(--warn-ink); border-color: var(--warn-line); }

  /* ファイルの置き場・入れ方タブ・貼り付け欄 */
  html[data-theme="dark"] .pj-drop { background: var(--surface-2); color: var(--muted); }
  html[data-theme="dark"] .pj-drop.drag { background: var(--ok-soft); color: var(--ok-ink); border-color: var(--ok-line); }
  html[data-theme="dark"] .pj-file { color: var(--ok-ink); }
  html[data-theme="dark"] .pj-tab { background: var(--surface-3); color: var(--muted); }
  ...
  html[data-theme="dark"] .pj-dup-ask select { background: var(--surface-2); color: var(--ink); border-color: var(--field-line); }

  /* 名簿に無かった人をその場で足す欄 */
  html[data-theme="dark"] .pj-miss-name { background: var(--surface-2); color: var(--ink); border-color: var(--field-line); }
  html[data-theme="dark"] .pj-miss-msg.ok { color: var(--ok-ink); }
  html[data-theme="dark"] .pj-miss-msg.ng { color: var(--danger-ink); }
  html[data-theme="dark"] .pj-summary .ok { color: var(--ok-ink); }
  html[data-theme="dark"] .pj-summary .ng { color: var(--danger-ink); }

  /* 取り込み内容の確認表 */
  html[data-theme="dark"] table.pj-table th { background: var(--surface-2); }
  html[data-theme="dark"] table.pj-table tr.row-ng td { background: var(--danger-soft); }
  html[data-theme="dark"] table.pj-table tr.row-ok td { background: var(--ok-soft); }
  html[data-theme="dark"] .pj-reason { color: var(--danger-ink); }
  html[data-theme="dark"] .pj-miss { color: var(--warn-ink); }
  html[data-theme="dark"] table.pj-table input[type="text"],
  html[data-theme="dark"] table.pj-table input[type="date"] { background: var(--surface-2); color: var(--ink); border-color: var(--field-line); }
  html[data-theme="dark"] table.pj-table input.edited { background: var(--ok-soft); border-color: var(--ok-line); }
  html[data-theme="dark"] table.pj-table input.pj-need { background: var(--warn-soft); border-color: var(--warn-line); }
  html[data-theme="dark"] table.pj-table input.pj-need::placeholder { color: #d9a94a; }
  html[data-theme="dark"] table.pj-table tr.row-skip td { background: var(--surface-2); color: var(--muted-dim); }

  /* すでに入っている案件との重なり */
  html[data-theme="dark"] .pj-dup-same { background: var(--info-soft); color: var(--info-ink); }
  ...
  <div class="pj-card">
    ...
    <ul class="pj-lead" style="padding-left:1.2em;">
      ...
      <li><b>運営人数がシートに書かれていない案件は、空のまま（黄色い枠）で出します。</b>
        この画面では<b>勝手に埋めません</b>（メンバー欄に並んでいる人数は<b>目安として薄字</b>で出します）。
        空のまま取り込むと、<b>参加人数とコンテンツから「仮」の人数を入れます</b>（2026-09-09 変更。案件のCSV取込と同じやり方）。
        <span class="muted">⚠ 入るのは「仮」です＝案件一覧に「仮」と出るので、決まったら本当の人数に直してください。
        どの案件に何名を入れたかは、取り込んだあとのメッセージに出します。<br>
        ⚠ 「並んでいる人数」では埋めません（2026-08-28）。それは実際に入った人の数で、必要人数ではないためです。<br>
        ⚠ 0のままにしないのは、運営人数が0だと「必要◯名／あと◯名」がぜんぶ0になり、
        <b>足りているように見えて</b>自動アサインの対象からも外れるためです。</span></li>
...
<script>
  ...
  function pjCountInput(r) {
    ...
    var title = (guess
      ...
      : 'シートに運営人数が書かれていません。')
      + '空のまま取り込むと、参加人数とコンテンツから「仮」の人数が入ります（あとで直せます）。';
   ...
   * ⚠ 空のまま入れると、サーバー側で「仮」の人数が入る（2026-09-09 baba決定）。
   *   出し方の正本＝App\Support\RequiredCountEstimate。ここに計算を書き写さない。
   */
  function pjConfirmEmptyCounts() {
    ...
    return confirm([
      ...
      '',
      'このまま取り込むと、参加人数とコンテンツから「仮」の人数を入れます',
      '（案件一覧に「仮」と出ます。決まったら本当の人数に直してください）。',
      '入れた人数は、取り込んだあとのメッセージに出します。',
      '',
