...
<style>
  ...
  /* ===== 黒ベース（ダークモード）のときの読みやすさ調整 =====
     白い面・茶色の文字・淡い色の帯を、暗い面＋明るい文字に置き換える。
  ...
  html[data-theme="dark"] .pi-card[style*="#f6fbf8"] { background: #1c2a22 !important; border-color: #2f5a41 !important; }
  html[data-theme="dark"] [style*="#6b5c49"],
...
<div class="pi-wrap">
  ...
  <div class="pi-card">
    ...
    <ol class="pi-steps">
      ...
      <li>1行に1人ずつ記入して保存（文字コードはそのままでOK＝BOM付きUTF-8）<br>
        <b>⚠ 2行目・3行目に入っている見本の行（山田花子・鈴木一郎）は、必ず消してください。</b>
        消し忘れたまま取り込むと架空の人が登録されます（メールが @example.com のままなら取込時に弾きます）。</li>
      <li>「② ファイルを選ぶ」で読み込み、下のプレビューで内容とエラーを確認</li>
      <li>問題なければ「③ この内容で登録」。<b>エラー行は登録されず、OK行だけ登録されます</b></li>
    </ol>
    <div class="pi-cols">
      <b>列：</b>
      種別（社員／スタッフ）※必須 ／ 氏名 ※必須 ／ ふりがな（五十音順の並びに使う・任意） ／ メール（重複不可） ／ 事務所（東京/大阪/名古屋/福岡/東北/札幌） ／
      チャットワークID（数字・任意） ／
      所属（社員のみ・<b>兼務は「／」か「,」で区切って複数書けます</b>。先頭が主な所属：
      イベプラ/セールス/クリエイティブ/経営管理/マーケティング/イベプロ/プロダクション/ビジパ/ARENA/あそ研） ／ 入社日（例 2025-04-01） ／ 通算経験回数（スタッフのみ・数字） ／
...
<script>
  ...
  function piParse(text) {
    ...
    var rows = lines.map(function (l) {
      ...
      return {
        type: get('種別'), name: get('氏名'), kana: get('ふりがな') || get('よみ') || get('読み'), email: get('メール'),
        cwid: get('チャットワークID'),
  ...
  function piValidate(r, seen) {
    ...
    if (r.cwid && !/^\d+$/.test(r.cwid)) e.push('チャットワークIDは数字');
    // テンプレートの見本行が残っていたら弾く（サーバー側と同じ基準）。
    // 見本のメールは example.com。本物の連絡先では使わないので、これで見つけられる。
    if (r.email && /@example\.(com|org|net)$/i.test(r.email)) e.push('見本の行が残っています（メールが@example.com）。消してから取り込んでください');
    return e;
  }
