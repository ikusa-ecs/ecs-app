  ...
  <nav>
    ...
    <div class="nav-group" data-group="アサイン">
      ...
      <div class="group-items">
        ...
        {{-- 配信・中継案件一覧＝配信／中継のある案件だけを並べ、配信担当を決める画面
             （2026-09-18・FBシート No.21 馬場さん）。 --}}
        <a class="{{ ($active ?? '') === 'broadcast_list' ? 'active' : '' }}" href="/broadcast-list"><span class="nav-icon">📡</span> 配信・中継案件一覧</a>
  ...
  <div class="userbox">
    @auth
      ...
      <div style="margin-top:8px; display:flex; flex-direction:column; gap:4px; font-size:12px;">
        <a href="/profile" style="color:var(--muted); text-decoration:underline;">マイプロフィール</a>
        <a href="/password" style="color:var(--muted); text-decoration:underline;">パスワード変更</a>
        <form method="POST" action="/logout" style="margin:2px 0 0;">
          @csrf
          <button type="submit" style="background:none; border:none; padding:0; color:var(--muted); cursor:pointer; font:inherit; text-decoration:underline;">ログアウト</button>
        </form>
      </div>
    @endauth
