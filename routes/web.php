...
Route::middleware(['auth', 'twofa', 'onboarded', 'tier:employee'])->group(function () {
    ...
    // ⚠ 2026-09-18 に共通設定からこの画面へ移した（baba要望）。保存先のキーは変えていない。
    Route::post('/active-bonus/settings', [ActiveBonusController::class, 'saveSettings']);
    ...
    // 経験回数（誰がどのコンテンツ・ポジションを何回やったか）。2026-08-28 baba要望で独立した画面に。
    // ⚠ 数え方の正本は App\Support\ExperienceCount（保存せず毎回数える）。
    Route::get('/experience', [ExperienceController::class, 'index']);
    Route::get('/experience/export.csv', [ExperienceController::class, 'exportCsv']);
