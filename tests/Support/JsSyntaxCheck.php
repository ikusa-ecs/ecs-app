...
class JsSyntaxCheck
{
    ...
    public static function problems(string $js): array
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            //   サーバーから渡す値が空になったときの形なので、必ず見つける。
            if ($prev === '=' && strpos(';,)]}', $c) !== false) {
                $problems[] = "{$line}行目：「=」のあとに何も無いまま「{$c}」が来ています。"
                    .'＝サーバーから渡す値が空になっています（データの文字化けなどで @json が失敗した形）。'
                    .'この画面のJavaScriptは丸ごと読み込みに失敗します。';
            }
